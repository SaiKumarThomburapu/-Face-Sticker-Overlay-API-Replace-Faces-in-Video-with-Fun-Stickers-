import cv2
import sys
import os
from src.entity.config_entity import StickerOverlayConfig, ConfigEntity
from src.logger import logging
from src.exceptions import CustomException

class StickerOverlay:
    def __init__(self, session_id: str):
        try:
            self.config = StickerOverlayConfig(config=ConfigEntity(), session_id=session_id)
            logging.info("StickerOverlay initialized")
        except Exception as e:
            raise CustomException(e, sys)

    def overlay(self, frame, bbox, sticker_img):
        try:
            # Create a copy of the frame to avoid modifying the original if something goes wrong
            output_frame = frame.copy()
            
            x1, y1, x2, y2 = bbox
            h, w = output_frame.shape[:2]

            x1 = max(0, x1)
            y1 = max(0, y1)
            x2 = min(w, x2)
            y2 = min(h, y2)

            face_w = x2 - x1
            face_h = y2 - y1

            if face_w <= 0 or face_h <= 0:
                logging.warning(f"Invalid bbox dimensions: {bbox}. Skipping overlay.")
                return output_frame

            sticker_resized = cv2.resize(sticker_img, (face_w, face_h))

            # Handle transparency (RGBA) vs no transparency (RGB)
            if sticker_resized.shape[2] == 4:  # RGBA image
                # Split the sticker into color and alpha channels
                overlay_bgr = sticker_resized[:, :, :3]
                alpha_mask = sticker_resized[:, :, 3:] / 255.0  # Normalize alpha to 0-1

                # Get the Region of Interest (ROI) from the frame
                roi = output_frame[y1:y2, x1:x2]
                
                # Blend the overlay with the ROI using the alpha mask
                blended_roi = (overlay_bgr * alpha_mask) + (roi * (1 - alpha_mask))
                
                # Place the blended ROI back into the frame
                output_frame[y1:y2, x1:x2] = blended_roi.astype(output_frame.dtype)

            else:  # RGB image (no transparency)
                output_frame[y1:y2, x1:x2] = sticker_resized[:, :, :3]

            logging.debug("Sticker overlaid successfully on one face")
            return output_frame

        except Exception as e:
            logging.error(f"Error in sticker overlay for bbox {bbox}: {str(e)}")
            # Return the original frame if overlay fails
            return frame




# import cv2
# import sys
# from src.entity.artifacts import StickerOverlayArtifact
# from src.entity.config_entity import StickerOverlayConfig, ConfigEntity
# from src.logger import logging
# from src.exceptions import CustomException

# class StickerOverlay:
#     def __init__(self):
#         self.config =StickerOverlayConfig(config=ConfigEntity())
#         logging.info("StickerOverlay initialized")

#     def overlay(self, frame, bbox, sticker_img):
#         try:
#             x1, y1, x2, y2 = bbox
#             h, w = frame.shape[:2]

#             x1 = max(0, x1)
#             y1 = max(0, y1)
#             x2 = min(w, x2)
#             y2 = min(h, y2)

#             face_w = x2 - x1
#             face_h = y2 - y1

#             if face_w <= 0 or face_h <= 0:
#                 return frame

#             sticker_resized = cv2.resize(sticker_img, (face_w, face_h))

#             if sticker_resized.shape[2] == 4:
#                 alpha_s = sticker_resized[:, :, 3] / 255.0
#                 alpha_l = 1.0 - alpha_s

#                 for c in range(3):
#                     frame[y1:y2, x1:x2, c] = (
#                         alpha_s * sticker_resized[:, :, c] +
#                         alpha_l * frame[y1:y2, x1:x2, c]
#                     )
#             else:
#                 frame[y1:y2, x1:x2] = sticker_resized[:, :, :3]

#             logging.info("Sticker overlaid successfully")
#             return StickerOverlayArtifact(frame=frame)
#         except Exception as e:
#             logging.error(f"Error in sticker overlay: {str(e)}")
#             raise CustomException(e, sys)



